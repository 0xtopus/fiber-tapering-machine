...
u16 LTDC_PanelID_Read(void)
{
	...
	GPIO_Initure.Pin = GPIO_PIN_5 | GPIO_PIN_9; // PB5, 9
	HAL_GPIO_Init(GPIOB, &GPIO_Initure);		// 初始化

	idx = (u8)HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_6);		 // 读取M0
	idx |= (u8)HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_5) << 1; // 读取M1
	idx |= (u8)HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_9) << 2; // 读取M2
	switch (idx)
	{
	case 0:
...
void HAL_LTDC_MspInit(LTDC_HandleTypeDef *hltdc)
{
	...
	// 初始化PG3，背光引脚
	GPIO_Initure.Pin = GPIO_PIN_3;			 // PG3推挽输出，控制背光
	...
	// 初始化PA3,4,6,8,9,11
	GPIO_Initure.Pin = GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_6 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_11;
	HAL_GPIO_Init(GPIOA, &GPIO_Initure);
	
	// 初始化PB5,8,9,10,11
	GPIO_Initure.Pin = GPIO_PIN_5 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11;
